...
if (fs.existsSync(masterJson)) {
  try {
    grouped = JSON.parse(fs.readFileSync(masterJson, "utf8"));
  } catch (e) {
    console.warn("⚠️ artikel/artikel.json rusak atau kosong, mulai dari kosong.");
    grouped = {};
  }
}
...
Object.values(grouped).forEach(arr => {
  arr.forEach(item => {
    existingFiles.add(item[1]); // posisi index 1 = filename
  });
});
...
console.log("✅ artikel.json & sitemap.xml selesai dibuat di root (merge dengan master).");
